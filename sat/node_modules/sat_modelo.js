...
exports.solve = function (fileName) {
  ...
  allAssignments = assignments(formula.variables)
...
function readFormula(fileName) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (text[i].charAt(0) == 'p') {
      ...
      numeroClausulas = text[i].split(' ')[3]
      numeroVariaveis = text[i].split(' ')[2]
    }
  ...
  //pegando o array inicial e passando para outro array sem posicoes vazias
  for (let i = 0; i < arrayClausulas.length; i++) {
    ...
    //troca novamente. array2 é o array das clausulas
    array2.push(array1)
    array1 = []
  ...
  //construir o array das clausulas
  let clausulas = array2
  //caso a linha p não exista, o programa roda normalmente
  if (!auxExistP) {
    numeroVariaveis = variaveis.length
    numeroClausulas = clausulas.length
  }
  ...
  //criando o objeto result
  let result = { 'clauses': [], 'variables': [] }
  //caso não haja problemas de quantidade de clausulas/variaveis  
  if (specOk) {
...
//Verifica, caso exista a linha p, se os numeros de clausulas e variaveis batem
function checkProblemSpecification(clausulasNumero, variaveisNumero, clausulas, variaveis) {
  if (clausulas.length != clausulasNumero || variaveis.length != variaveisNumero) {
    console.log("Numero de variaveis ou clausulas incorreto")
    return false
  } else {
    return true
  }
